const xhr = new XMLHttpRequest();
const CONTENT = "https://api.hnpwa.com/v0/item/@id.json";
const NEWS = "https://api.hnpwa.com/v0/news/1.json";
const container = document.querySelector("#title");
const divcontent = document.createElement("div"); //제목에 대한 상세내용
//@id 게시물마다의 키값
//페이징 처리 -1step
const store = {
  currentPage: 1, //현재 내가 바라보는 페이지 번호 (변하니까 상태)
};
//아래 함수의 사용처는 두군데
//요청 URL만 바뀌고 나머지는 같다 파라미터에 url을 줬기때문에
function getData(url) {
  xhr.open("GET", url, false);
  xhr.send();
  return JSON.parse(xhr.responseText);
}
//뉴스목록 가져오기
function newsList() {
  const news = getData(NEWS);
  //태그를 담을 배열선언
  const getList = [];
  //태그 담기전에  ul태그 먼저 담기
  const div = document.createElement("div");
  getList.push(`<ul>`);
  for (let i = (store.currentPage - 1) * 10; i < store.currentPage * 10; i++) {
    getList.push(`        <li>    
    <a href='#${news[i].id}'></a>   
     ${news[i].title}(${news[i].comments_count})  
       </li>
    `);
  } //end of for
  //배열의 맨 뒤에 추가하는 함수가 push
  getList.push("</ul>");
  getList.push("<div>");
  //변수 사용할거면 `` 써야해 (백틱)
  getList.push(    `<a href='#'/page/${      store.currentPage > 1 ? store.currentPage - 1 : 1    }>이전페이지</a>`
  );
  getList.push("&nbsp;&nbsp;"); // 링크사이 띄우기
  getList.push(`<a href='#'/page/${store.currentPage + 1}>다음페이지</a>`);
  getList.push("</div>");
  //빈문자열 주면 구분없는 하나의 합쳐진 HTML문자열을 얻음
  container.innerHTML = getList.join("");
}

//뉴스 상세 내용 보기
function newDetail() {
  const id = this.location.hash.substring(7); //상세보기 앞에 #/show/가 있으니까
  const newsContent = getData(CONTENT.replace("@id", id));
  container.innerHTML = `
<h1> ${newsContent.title}</h1>
<div>
<a href="#/page/${store.currentPage}">목록</a>;
</div>
  `;
}
//라우터에서 화면 전환하기
function router() {
  //글 목록의 링크는# 그러면 location.href에 #이 들어왔을 텐데
  //왜 참???? location.href에 #만 들어오면 빈값을 반환
  //그래서 작동
  const routePath = location.hash;
  //첫 진입일 때는 newsList를 호출
  if (routePath === "") {
    //처음 요청시에는 해시값이 없으니까 여기가 실행된다
    newsList();
  } else if (routePath.indexOf("#/page") >= 0) {
    //해시값중에서 숫자값만 추출하기
    //#을 제거하고 문자열 길이를 계산 후 문자열을 숫자로 바꾸기
    store.currentPage = Number(routePath.substring(7));
    newsList();
  } else {
    newsDetail();
  }
} //end of router
window.addEventListener("hashchange", router);
router();
